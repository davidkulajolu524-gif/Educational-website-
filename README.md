...
This project is ready for Render or any host that runs the `Procfile`. Set the service to install from `requirements.txt` and use `uvicorn backend:app --host 0.0.0.0 --port $PORT`.

### Render

Create a new Blueprint from this repository. Render will read `render.yaml`, install the requirements, start the FastAPI service, and check `/api/health`. Add the SMTP environment variables requested by the Blueprint to enable password-reset and contact emails. The local SQLite filesystem may be temporary on hosted services, so use a persistent database before relying on account data in production.

The contact endpoint saves submissions locally and can send email notifications when SMTP is configured.
